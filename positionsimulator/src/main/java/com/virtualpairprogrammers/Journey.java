...
/**
 * A callable (so we can invoke in on a executor and join on it) that sends message
 * to a queue periodically - represetting the journey of a delivery vehicle
...
public class Journey implements Callable<Object> {
	...
	public Object call() throws InterruptedException {
		...
		for (String nextReport: this.positions) {
			...
			positionMessage.put("vehicle", this.vehicleName);
			...
			// we have an element of randomness to help the queue be nicely distributed
	...
	/**
	 * Sends a message to the position queue - we've hardcoded this in at present ofc
